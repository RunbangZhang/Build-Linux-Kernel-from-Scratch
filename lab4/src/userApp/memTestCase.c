...
int test_kmalloc(int argc, unsigned char **argv){  
	//======for kmalloc===============================
	...
	*(buf2+23) = '\000';
	...
	myPrintk(0x5, "BUF1(size=19, addr=0x%x) filled with 17(*): ",(unsigned long)buf1);
	myPrintk(0x7,buf1);
	myPrintk(0x5, "BUF2(size=24, addr=0x%x) filled with 22(#): ", (unsigned long)buf2);
	myPrintk(0x7,buf2);
...
void memTestCaseInit(void){	
	...
	addNewCmd("testKmalloc\0", test_kmalloc, NULL, "Kmalloc, write and read.\0");
}

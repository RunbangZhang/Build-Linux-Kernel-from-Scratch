...
void testSchedulerPRIORITY0(void){  //FCFS or RR or SJF or PRIORITY0
	tskPara tskParas[4];
	int i;
	...
	setTskPara(ARRTIME,250,&tskParas[0]);
	setTskPara(PRIORITY,10,&tskParas[0]);
	createTsk(myTSK0,&tskParas[0]);

	setTskPara(ARRTIME,250,&tskParas[1]);
	setTskPara(PRIORITY,5,&tskParas[1]);
	createTsk(myTSK1,&tskParas[1]);

	setTskPara(ARRTIME,250,&tskParas[2]);
	setTskPara(PRIORITY,7,&tskParas[2]);
	createTsk(myTSK2,&tskParas[2]);

	initShell();
    memTestCaseInit();
    setTskPara(ARRTIME,520,&tskParas[3]);
	setTskPara(PRIORITY,12,&tskParas[3]);
